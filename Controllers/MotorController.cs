...
public class MotorController : ControllerBase {
    ...
    [HttpGet("/status")]
    [HttpPost("/status")]
    [ProducesResponseType(typeof(List<MotorStatusModel>), 200)]
    public IActionResult GetStatus() {
        var result = new List<MotorStatusModel>();
        var positions = _motor.GetPosition();
        var states = _motor.GetState();
        for (int i = 0; i < _motor.Count; i++) {
            result.Add(
                new MotorStatusModel {
                    Id = i + 1,
    ...
    [ProducesResponseType(typeof(StatusOk), 200)]
    [ProducesResponseType(typeof(ErrorResult), 400)]
    public async Task<IActionResult> Manual([FromBody] MotorCommandRequest command) {
        var posList = command.Position;
